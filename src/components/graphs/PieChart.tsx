...
const colorsHex = ['#808080', '#EAB308', '#6366F1', '#8B5CF6', '#FF00FF'];
...
export default function PieChart({assets, nav}: {assets?: {name: string, value: number}[], nav: number}) {
  ...
  const porcentage = (number: number) => (number / nav) * 100;
  const sortedAssets = assets?.sort((a, b) => porcentage(b.value) - porcentage(a.value));
